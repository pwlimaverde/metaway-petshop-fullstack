async def test_clients_full_crud_admin(client, seed_data, auth_headers) -> None:
    ...
    created = await client.post(
        "/api/v1/clients",
        headers=admin,
        json={"name": "Cliente Novo", "cpf": "11122233344"},
    )
...
async def test_clients_not_found_branches(client, seed_data, auth_headers) -> None:
    ...
    assert (
        await client.patch(
            "/api/v1/clients/999999", headers=admin, json={"name": "X"}
        )
    ).status_code == 404
    assert (
...
async def test_duplicate_client_cpf(client, seed_data, auth_headers) -> None:
    admin = await auth_headers(seed_data["admin_cpf"], seed_data["admin_password"])
    response = await client.post(
        "/api/v1/clients",
        headers=admin,
        json={"name": "Duplicado", "cpf": seed_data["client_cpf"]},
    )
    assert response.status_code == 400


async def test_me_not_linked(
    client, seed_data, auth_headers, unlinked_client_user
) -> None:
    headers = await auth_headers(
        unlinked_client_user["cpf"], unlinked_client_user["password"]
    )
    assert (await client.get("/api/v1/clients/me", headers=headers)).status_code == 404
    assert (
        await client.patch(
            "/api/v1/clients/me", headers=headers, json={"name": "X"}
        )
    ).status_code == 404
